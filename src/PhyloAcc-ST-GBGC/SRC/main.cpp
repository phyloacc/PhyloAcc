...
double ratio0 = -2; //initial conserved selection
double ratio1 = 0; // initial accelerated selection
double missing_thres = 0.8;

double nprior_a = 5, nprior_b = 0.4;  //around 0, shifted gamma, >=-2
double cprior_a = 5, cprior_b = 0.4;  //around -2, inverse and shift gamma (-x)

double nGBprior_a = 4, nGBprior_b = 0.8, nGBprior_c = 0.2;  //5, 0.6, around 3, c: prob of B==0
double cGBprior_a = 1, cGBprior_b = 1;  //around 

int ropt = 0;  // no bounds on selection coefficient (already imposed by prior)
double cub = 1;
double nlb = 0.6;
...
void LoadParams(int argc, char* argv[])
{
    ...
    if (argc > 1)
        ...
        params_path = "/Users/hzr/Phd_2/Bird/PhyloAcc_v1-2/paramR-2-3.txt"; //params2
...
int main(int argc, char* argv[])
{
    ...
    BPP bpp(0, profile, phytree, output_path, targetspecies, outgroup, conserve_prop, conservegroup, refspecies, ratio0, ratio1, ropt, cub, nlb, nprior_a, nprior_b, cprior_a, cprior_b,nGBprior_a, nGBprior_b, nGBprior_c, cGBprior_a, cGBprior_b, seed, prep_lrate, prep_grate, consToMis, nconsToMis, missing_thres, sample_indel);  //c=1 test run first element
    ...
    out_Z0 << "No.\tn_rate\tc_rate\tgBC"; out_Z1 << "No.\tn_rate\tc_rate\tgBC"; out_Z2 << "No.\tn_rate\tc_rate\tgBC";
    
    for(int s=0; s<bpp.N;s++){
        out_Z0 <<"\t"<<bpp.nodes_names[s]<<"_B";
        out_Z1 <<"\t"<<bpp.nodes_names[s]<<"_B";
        out_Z2 <<"\t"<<bpp.nodes_names[s]<<"_B";
    }
    ...
    for(int iter =0; iter<num_chain; iter++)
    {
        ...
        for(std::size_t i = 0; i <ids.size(); i++ ) // ids.size() bpp.C
        {
            int c = ids[i];
            //c = 2058;
            bool filter = false;
            //if(bppc.root<77 || bppc.parent2[76] == -1) continue;  // too many species missing
            cout << "Computing element " << c << " ..." << endl;
            // null model
            try{
            BPP_C bppc(c, profile, bpp, gapchar, missing_thres, filter, verbose);  // for individual element
            if(filter) {
