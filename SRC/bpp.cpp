...
void BPP::sample_hyperparam(double lrate_prop, double grate_prop, vector<int> & ids) // recompute log_TM, double indel_prop, double indel2_prop, 
{
...
//
    ...
    //MH to sample lrate and grate together
    //lrate
    double MH_ratio =0;
    ...
    double alphasp[3] = {grate_prop*vgr, lrate_prop*vgr, (1 - grate_prop - lrate_prop)*vgr};
    double thetasp[3] = {ind_grate, ind_lrate, 1 - ind_grate - ind_lrate};
    
    double alphas[3] = {ind_grate*vgr, ind_lrate*vgr, (1 - ind_grate - ind_lrate)*vgr};
    double thetas[3] = {grate_prop, lrate_prop, 1 - grate_prop - lrate_prop};
    
    MH_ratio += gsl_ran_dirichlet_lnpdf(3,alphasp, thetasp) - gsl_ran_dirichlet_lnpdf(3,alphas, thetas);
    //MH_ratio += gsl_ran_dirichlet_lnpdf(3,prior_glr, thetas) - gsl_ran_dirichlet_lnpdf(3,prior_glr, thetasp); // prior
    ...
    cout << "lgrate: " << MH_ratio << ", " << ind_grate << ", " << grate_prop << ", " << ind_lrate << ", " << lrate_prop << endl << endl;
    ...
    if(log(gsl_rng_uniform(RNG)) < MH_ratio)
    {
        ...
        for(int s=0; s<N; s++)
        {
            ...
            TM_Int[s](0,0) = 1 - ind_grate - ind_lrate;
            TM_Int[s](1,0) = ind_grate;
            TM_Int[s](2,0) = ind_lrate;
