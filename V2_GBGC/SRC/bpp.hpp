...
private:
    ...
    vector<vector < vector <bool> >> Max_B;
    vector<vector < vector <int > >> cur_Z;
    vector < vector <double> > cur_B;
    ...
    vector< double > cur_nGB;
    vector< double > cur_cGB;
    ...
    double nprior_a, nprior_b, nprior_c;  //> -2, shift gamma by nprior_c
    double cprior_a, cprior_b;  //around -2 , -gamma
    
    double nGBprior_a, nGBprior_b, nGBprior_c;  //around 3, gamma
    double cGBprior_a, cGBprior_b;  //around 0, expoential
...
BPP(int pC, PhyloProf & _prof, PhyloTree & _tree, string output_path, string _target, string _outgroup, double _conserve_prop, string _conservegroup, string _refspecies, double _ratio0, double _ratio1, int _ropt, double _cub, double _nlb, double _npriora, double _npriorb, double _cpriora, double _cpriorb,double _nGBpriora, double _nGBpriorb, double _nGBpriorc, double _cGBpriora, double _cGBpriorb, int _seed, double _prep_lrate, double _prep_grate, double _consToMis, double _nconsToMis, double missing_thres, bool _sample_indel)
{
    ...
    nprior_a = _npriora, nprior_b = _npriorb;  nprior_c = 2; //around 1
    cprior_a = _cpriora, cprior_b = _cpriorb;  //around ratio
    
    nGBprior_a = _nGBpriora, nGBprior_b = _nGBpriorb, nGBprior_c = _nGBpriorc;  //around 1
    cGBprior_a = _cGBpriora, cGBprior_b = _cGBpriorb;  //around ratio
...
//        }
    ...
    log_TM_Int = vector<mat >(N, zeros<mat>(6,3));
   ...
   // cout << endl;
    ...
    for(int s=0; s<N; s++){
        log_TM_Int[s].rows(0, 2) = log(TM_Int[s]);
        log_TM_Int[s].rows(3, 5) = log(TM_Int[s]);
    }
...
static vec log_exp_rowsum(mat log_y)
{
    double log_ymax = log_y.max();
    log_y = log_y - log_ymax;
    
    vec result = log(sum(exp(log_y), 1));
    return(result + log_ymax);
}
